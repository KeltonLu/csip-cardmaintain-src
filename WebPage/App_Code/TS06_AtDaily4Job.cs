...
/// <summary>
/// TS06_AtDaily4Job 的摘要描述
/// </summary>
public class TS06_AtDaily4Job
{
    ...
    public TS06_AtDaily4Job(string jobID)
    {
        this.jobID = jobID;
        JobHelper.strJobID = jobID;
    }

    public TS06_AtDaily4Job(string jobID, EntityAGENT_INFO eAgentInfo)
    {
        this.jobID = jobID;
    ...
    public string DownloadFromFTP(string date, string localPath, string extension, ref bool isDownload, ref string unZipPwd, ref string errorMsg)
    {
        ...
        try
        {
            ...
            {
                isDownload = false;
                errorMsg = "取得 JOB tblFileInfo 相關資料失敗，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                return "";
            }
            ...
            fileName = string.Format("TS06{0}.{1}", date, extension);
    ...
    public bool CheckImportLog(string date, string fileName, ref string errorMsg)
    {
        ...
        try
        {
            ...
            {
                errorMsg = "查詢時發生錯誤，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                return false;
            }
            ...
            if (!isSuccess)
            {
                errorMsg = "INSERT Import_Log 失敗！請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
            }
        }
        catch (Exception ex)
        {
    ...
    public bool TruncateTable(string tableName, ref string errorMsg)
    {
        ...
        {
            errorMsg = "Truncate 時發生錯誤，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
        }
        return result;
    }

    /// <summary>
    /// 檢查CPMAST4_TMP資料的日期是否正確，若否則複製CPMAST4_TMP的錯誤資料到CPMAST4_ERR並刪除CPMAST4_TMP中的錯誤資料
    /// </summary>
    /// <param name="fileName">檔案名稱</param>
    /// <param name="errorDataCount">錯誤資料筆數</param>
    /// <param name="errorMsg">錯誤訊息</param>
    /// <returns></returns>
    public bool CheckCpmast4Tmp(string fileName, ref int errorDataCount, ref string errorMsg)
    {
        bool checkResult = false;
        try
        {
            const string sqlSearhText = @"SELECT COUNT ( * ) FROM	CPMAST4_TMP WHERE ( len( ltrim( MAINT_D ) ) = 8 	AND ( SUBSTRING ( MAINT_D, 3, 1 ) <> '/' OR SUBSTRING ( MAINT_D, 6, 1 ) <> '/' ) )";

            DataHelper dh = new DataHelper();
            DataSet ds = dh.ExecuteDataSet(sqlSearhText);
            if (ds == null || ds.Tables.Count <= 0)
            {
                errorMsg = "查詢 CPMAST4_TMP 發生錯誤，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                return false;
            }
            ...
            {
                //將錯誤資料另存至CPMAST4_Err，並從CPMAST4_TMP刪除錯誤資料
                if (!CopyDataToCpmast4ErrAndDeleteCpmast4Tmp(fileName, ref errorMsg))
                {
                    if (string.IsNullOrEmpty(errorMsg))
                        errorMsg = "將錯誤資料另存至CPMAST4_Err，並從CPMAST4_TMP刪除錯誤資料時發生錯誤";
                    return false;
                }
                else
                {
        ...
        {
            Logging.Log(ex.Message);
            JobHelper.SaveLog("將錯誤資料另存至CPMAST4_Err，並從CPMAST4_TMP刪除錯誤資料時發生例外錯誤：" + ex.Message);
            errorMsg = "將錯誤資料另存至CPMAST4_Err，並從CPMAST4_TMP刪除錯誤資料時發生例外錯誤";
            return false;
        }
        return checkResult;
    }
    ...
    /// <summary>
    /// 複製CPMAST4_TMP的錯誤資料到CPMAST4_ERR並刪除CPMAST4_TMP中的錯誤資料
    /// </summary>
    /// <param name="fileName">檔案名稱</param>
    /// <param name="errorMsg">錯誤訊息</param>
    /// <returns></returns>
    public bool CopyDataToCpmast4ErrAndDeleteCpmast4Tmp(string fileName, ref string errorMsg)
    {
        bool copyAndDeleteStatus = false;
        try
        {
            const string sqlCopyText = @"
            INSERT INTO CPMAST4_Err ( TYPE, CUST_ID, CARD_TYPE, FLD_NAME, BEFOR_UPD, AFTER_UPD, LST_LIMIT, CUR_LIMIT, MAINT_D, MAINT_T, USER_ID, TER_ID, EXE_Name ) 
            SELECT
            TYPE,
            CUST_ID,
            CARD_TYPE,
            ...
            const string sqlDelText = @"DELETE FROM CPMAST4_TMP WHERE ( len( ltrim( MAINT_D ) ) = 8 	AND ( SUBSTRING ( MAINT_D, 3, 1 ) <> '/' OR SUBSTRING ( MAINT_D, 6, 1 ) <> '/' ) )";

            SqlCommand sqlcmd = new SqlCommand();
            sqlcmd.CommandText = sqlCopyText;
            sqlcmd.Parameters.Add(new SqlParameter("@fileName", fileName));
            //新增錯誤資料至CPMAST4_ERR
            copyAndDeleteStatus = CSIPCardMaintain.BusinessRules.BRCPMAST4_Err.Add(sqlcmd);

            if (!copyAndDeleteStatus)
            {
                errorMsg = "新增錯誤資料至 CPMAST4_ERR 失敗，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                return false;
            }
            ...
            if (ds == null)
            {
                errorMsg = "刪除CPMAST4_TMP錯誤資料失敗，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                return false;
            }
            else
            {
        ...
        {
            Logging.Log(ex.Message);
            JobHelper.SaveLog("複製CPMAST4_TMP的錯誤資料到CPMAST4_ERR並刪除CPMAST4_TMP中的錯誤資料時發生例外錯誤：" + ex.Message);
            errorMsg = "複製CPMAST4_TMP的錯誤資料到CPMAST4_ERR並刪除CPMAST4_TMP中的錯誤資料時發生例外錯誤";
            return false;
        }
    ...
    /// <param name="errorMsg">錯誤訊息</param>
    /// <returns></returns>
    public bool ConvertCpmast4TmpData(ref int correctDataCount, ref string errorMsg)
    {
        try
        {
            const string sqlSearchText = @"select count(*) from  CPMAST4_TMP";
            const string sqlUpdateText1 = @"update CPMAST4_TMP Set MAINT_D = '20' + substring( MAINT_D,7,2) + substring( MAINT_D,1,2) + substring( MAINT_D,4,2) where len(ltrim(MAINT_D))=8";
            const string sqlUpdateText2 = @"update CPMAST4_TMP Set MAINT_D = ''  where len(ltrim(MAINT_D))<>8";
            const string sqlUpdateText3 = @"update CPMAST4_TMP Set USER_ID = ''  where USER_ID is null";
            DataHelper dh = new DataHelper();
            DataSet ds = new DataSet();
            ...
            if (ds == null || ds.Tables.Count <= 0)
            {
                errorMsg = "查詢 table[CPMAST4_TMP]時發生錯誤，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                return false;
            }
        ...
        {
            Logging.Log(ex.Message);
            JobHelper.SaveLog("轉換 CPMAST4_TMP 日期格式時發生例外錯誤：" + ex.Message);
            errorMsg = "轉換 CPMAST4_TMP 日期格式時發生例外錯誤";
            return false;
        }
    }
    ...
    /// <param name="errorMsg">錯誤訊息</param>
    /// <returns></returns>
    public bool InsertCorrectDataToCpmast4(string fileName, ref string errorMsg)
    {
        try
        {
            //查詢 CPMAST4_TMP有無資料
            const string sqlSearhText = @"SELECT COUNT ( * ) FROM	CPMAST4_TMP";
            ...
            INSERT INTO CPMAST4 ( TYPE, CUST_ID, CARD_TYPE, FLD_NAME, BEFOR_UPD, AFTER_UPD, LST_LIMIT, CUR_LIMIT, MAINT_D, MAINT_T, USER_ID, TER_ID, EXE_Name ) 
            SELECT
            TYPE,
            CUST_ID,
            CARD_TYPE,
            ...
            if (result <= 0)
            {
                JobHelper.SaveLog("table[CPMAST4_TMP]無資料！", LogState.Info);
                return true;
            }
            else
            {
                if (!CSIPCardMaintain.BusinessRules.BRCPMAST4.Add(sqlcmd))
                {
                    errorMsg = "新增正確資料至table[CPMAST4]時發生錯誤，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                    return false;
                }
            }
            return true;
        ...
        {
            Logging.Log(ex.Message);
            JobHelper.SaveLog("將正確資料 Insert 到 CPMAST4 時發生例外錯誤：" + ex.Message);
            errorMsg = "將正確資料 Insert 到 CPMAST4 時發生例外錯誤";
            return false;
        }
    }
    ...
    public bool UpdateImportLog(int correctDataCount, int errorDataCount, int datErrorDataCount, string date, ref string errorMsg)
    {
        try
        {
            if (correctDataCount > 0)
            {
                ...
                sqlcmd.Parameters.Add(new SqlParameter("@INDate", date));
                sqlcmd.Parameters.Add(new SqlParameter("@fileName", string.Format("TS06{0}.EXE", date)));
                bool updateStatus = CSIPCardMaintain.BusinessRules.BRImprot_Log.Update(sqlcmd);
                if (!updateStatus)
                {
                    errorMsg += "　更新table[Import_log]失敗，請確認 JobLog(Log\\JobTS06_AtDaily4Job\\) 或 DefaultLog(Log\\Default\\)";
                    return false;
                }
            }
            else
    ...
    private DataTable ValidateFileLength(FileInfo file, int filerightlength, ref string errorMsg, ref bool makeErrorFile, ref string errorFilePath, ref int errorDataCount, ref int datDataCount)
    {
        ...
        try
        {
            ...
            if (intcounterror > 0)
            {
                //將錯誤資料寫入檔案 TSyyyyMMdd.dat_ERROR.TXT
    ...
    /// <param name="errorMsg">錯誤訊息</param>
    /// <returns></returns>
    public bool InsertCpmast4Tmp(string tableName, DataTable sourceData, ref string errorMsg)
    {
        bool result = false;
        ...
        {
            Logging.Log(ex.Message);
            JobHelper.SaveLog("將資料匯入 CPMAST4_TMP 時發生例外錯誤：" + ex.Message);
            errorMsg = "將資料匯入 CPMAST4_TMP 時發生例外錯誤";
            return false;
        }
        finally
        {
